        ...
        if int(cruse)==0:
            ...
            if (float(time.time())-float(lasttime))>0.5:
                ...
                if int(rea[13])==1:
                    cruse=1
                    crusespeed=int(urpm)
                    cruseuj=updown(rea[5],rea[6])
            else:
                sig="n"
            return (dast+sig+">")
        if cruse==1:
            if wasd(rea[0],rea[1])!=None and int(rpm)!=0 :
                dast=(wasd(rea[0],rea[1])+rpm+cruseuj+str(crusespeed))
                sat=2
            if wasd(rea[0],rea[1])=='x' or int(rpm)==0 or int(rea[7])!=0 or int(rea[8])!=0 or int(rea[9])!=0 or int(rea[10])!=0: 
                if sat!=0 or int(rea[7])!=0 or int(rea[8])!=0 or int(rea[9])!=0 or int(rea[10])!=0:
                    dast=('x00'+cruseuj+str(crusespeed))
                    sat-=1
            if (float(time.time())-float(lasttime))>0.5:
                if int(rea[7])==1:
                    sig="n"
            ...
            return (dast+sig+">")
        if cruse==2:
            if(int(crusespeed>=10) and float(crusetime)-float(time.time())>=0.1):
                crusespedd-=10
                crusetime=time.time()
                if wasd(rea[0],rea[1])!=None and int(rpm)!=0 :
                    dast=(wasd(rea[0],rea[1])+rpm+cruseuj+str(crusespeed))
