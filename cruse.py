...
try:
    def make_command(rea):
        ...
        sig="n"
        if wasd(rea[0],rea[1])!=None and int(rpm)!=0 or updown(rea[5],rea[6],cruse)!='i' :
        ...
        if wasd(rea[0],rea[1])=='x' or int(rpm)==0 or int(rea[7])!=0 or int(rea[8])!=0 or int(rea[9])!=0 or int(rea[10])!=0:
            if sat!=0 or int(rea[7])!=0 or int(rea[8])!=0 or int(rea[9])!=0 or int(rea[10])!=0:
                dast=('x00'+updown(rea[5],rea[6],cruse)+urpm)
                sat-=1
        if (float(time.time())-float(lasttime))>0.5:
            ...
            if int(rea[13])==1:
                if(!cruse):
                    cruse=True
                    cruse_s=urpm
                    cruse_d=updown(rea[5],rea[6],False)
    ...
    def updown(x,y,check):
        if y>=0.5 and abs(x)<0.3 and not check:
            return 'u'
        if y<=-0.5 and abs(x)<0.3  and not check:
            return 'j'
            if(!check):
    ...
    class XboxController(object):
        ...
        def read(self): # return the buttons/triggers that you care about in this methode
            ...
            bl=self.LeftDPad
            br=self.RightDPad
            return [lx, ly, a, b, rt,rx,ry,y,b,a,x,bb,lt,rb]
